...
public class MainController {
    ...
    public String index(@ModelAttribute("savetravels") savetravelsmodel savetravels,Model model ) {
	 ...
	 @PostMapping("/expenses")
	    public String createSaveTravels(@Valid @ModelAttribute("savetravels") savetravelsmodel savetravels, BindingResult result) {
	        if (result.hasErrors()) {
	            return "index.jsp";
	        } else {
	 ...
	 @RequestMapping("/expenses/{id}/edit")
	  ...
	  public String update(@Valid @ModelAttribute("travels")  savetravelsmodel savetravels, BindingResult result, Model model) {
	    if (result.hasErrors()) {
	      model.addAttribute("savetravels", savetravels);
	      return "edit.jsp";
	    } else {
